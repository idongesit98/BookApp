...
class LawBookUseCase @Inject constructor(val bookRepository: BookRepository) {
    operator fun invoke():Flow<Resource<Books>> = flow {
        try {
            emit(Resource.Loading())
            val lawBooks = bookRepository.getLawBooks().toBookDto()
            emit(Resource.Success(lawBooks))
