...
class BookDetails @Inject constructor(val bookRepository: BookRepository) {
    operator fun invoke(id:String):Flow<Resource<Books>> = flow {
        try {
            emit(Resource.Loading())
            val Books = bookRepository.getBookDetails(id).toBookDto()
