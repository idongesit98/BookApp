...
class SearchBooksUseCase @Inject constructor(val bookRepository: BookRepository) {
    operator fun invoke(authororBookName:String):Flow<Resource<Books>> = flow {
        try {
            emit(Resource.Loading())
            val searchBooks = bookRepository.getSearchBook(authororBookName).toBookDto()
            emit(Resource.Success(searchBooks))
