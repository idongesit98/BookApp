...
class ScienceBookUseCase @Inject constructor(val bookRepository: BookRepository) {
    operator fun invoke(): Flow<Resource<Books>> = flow {
        try {
            emit(Resource.Loading())
            val scienceBooks = bookRepository.getScienceBooks().toBookDto()
            emit(Resource.Success(scienceBooks))
