...
class EnglishUseCase @Inject constructor(val bookRepository: BookRepository){
    operator fun invoke(): Flow<Resource<Books>> = flow {
        try {
            emit(Resource.Loading())
            val englishBooks = bookRepository.getEnglishBooks().toBookDto()
            emit(Resource.Success(englishBooks))
